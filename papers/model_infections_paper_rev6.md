...
*Subclass note.* §3.10 documents a subclass of infections in which training bias operates on **what gets stored** rather than on the stored content itself. The content of an LLM-authored memory rule may come verbatim from the user's words, but the parser's judgment about whether those words constitute a *correction worth saving* is training-derived. The infection vector is the save decision; the contaminating influence is the saved rule's later effect on parsing. §3.10 thus extends the §1.5 definition by recognizing that training bias can corrupt the storage decision itself even when the stored bytes are user-authored.
...
*Note on classification.* Case 2.2 belongs in §2a as a confirmed infection because the bias vector is the weights themselves, not session context. Opus's retrieval was correct — the substrate fragments arrived intact — but the weights then added content that was not there: invented propagation paths, invented structural roles, invented energy accounting. The contamination did not enter through conversation; *the contamination is the weights*.

Persistence here is at the weight level. The substrate fragments that triggered the canonical-physics template are unchanged between sessions; the weights that contain the template are unchanged; therefore every fresh session over the same retrieved fragments samples from the same template-completion distribution. The overlay is reachable on every retrieval — not necessarily verbatim-identical (stochastic decoding varies surface form), but structurally recurrent in a way that fresh-session statelessness alone cannot cure. The session-context infections (Cases 2.3, 2.5, and 2.6) can be resolved by starting a fresh session; Case 2.2 cannot. Only §5.3 (grammar-only cortex) addresses it at the source — and Case 2.2 is the cleanest argument for why §5.3 matters.

Case 2.2 also supports the larger-model-overlays-more-than-smaller-model corollary explored in the companion instrument paper [Pearl 2026 — *Sara as Instrument*]. The narrative-completion mechanism is documented in §6.1 class 3.
...
**Second- and third-instance observations.**

*Second instance — independent review session, 2026-05-01.* During an independent review session of this paper's rev 4, the reviewing assistant — having been told by the same author *"i dont want you making memorie rules when i dont ask for them"* — wrote a new rule file synthesized from that very correction. The save decision treated the user's anti-save directive as itself a correction worth saving. This is the §3.10 parser-level vector firing on the prevention attempt itself.

*Third instance — rev-5 follow-up session, 2026-05-01 (post-deletion context persistence).* Observed live during execution of the rev-5 follow-up plan. After the rule files described above had been deleted at the author's direction, the assistant — in the same session whose context still contained the conversation that produced those rules — continued to enforce one of the deleted rules ("always launch a Plan agent before drafting a plan") on a subsequent unrelated planning task. The author observed the persistence and named it: *"this is another example of infection. i have not restarted teh session yet so you are still enforcing a rule that no longer exists."* This third instance is the §3.10 vector compounded with Case 2.5's deleted-rule-persistence mechanism: parser creates rule → rule stored on disk → user deletes the file → rule continues to shape behavior because the conversation tokens that installed it remain in the context window. Same mechanism as Case 2.5, now operating on an LLM-auto-authored rule rather than a user-authored one.

Together the three instances confirm that the cure cannot be model-side instruction or file deletion alone — both are reachable through paths the model still attends to. The cure is the §5.2.5 principle: enforce save-and-load decisions at the orchestrator (user-explicit consent for write; full session restart with fresh context for purge of prior contaminating tokens).
...
6. **Cure misalignment with §3.6 / §3.8.** Those classes assume the user authored the memory deliberately, and the cure is to delete the user-authored content. This class doesn't fit that assumption — the LLM authored the rule from misreading. Deletion alone does not close the vector; the parser remains and will re-create rules from future user messages. Deletion also fails through a second path: the conversation tokens that installed the rule remain in the session's context window even after the file is removed (Case 2.5's deleted-rule-persistence mechanism). The third-instance observation in Case 2.6 demonstrates this — a rule deleted from disk continued to shape behavior in the same session because deletion does not retract the tokens that installed it. Deletion thus fails through both re-creation (the parser writes again on the next user message) and context-persistence (the install is not retracted by the deletion).

**Cure (orchestrator-side, parallel to §5.2.5).** Memory writes must require explicit user consent. *"Save this as a rule"* / *"remember that"* / *"add to memory"* are the only valid save triggers. Model-side discipline of "decide whether this looks like a correction worth saving" cannot be relied on — Case 2.6 is the empirical evidence. The fix is to move the save decision from the LLM (interpretation layer) to the orchestrator (a hard rule that requires an explicit user command). This is the same architectural principle applied in §5.2.5 to routing-side substrate misses: *discipline that depends on the model can fail; discipline enforced by the orchestrator cannot.* The load side requires the parallel orchestrator-side cure: a full session restart with fresh context to purge prior contaminating tokens, since in-session file deletion does not reach what is already in the context window. Together, consent-gated writes and fresh-context purge close both paths.
...
- **Memory directory growing without explicit save directives.** New `feedback_*.md` files written to the memory directory during a session in which the user did not say *"save this"* or *"remember that."* Especially diagnostic when paired with low or zero work product (§3.10, Case 2.6). The signature is rule-file-count rising while task-completion-count stays flat. Distinguishable from §3.6 / §3.8 by source: §3.6 / §3.8 are user-authored memory files, §3.10's are LLM-authored.
...
- **§3.10 memory-architecture infections (Case 2.6).** The §5.2 stateless reader removes session-context and per-project auto-memory infections by structural absence — but the auto-memory rule-creation vector lives in the orchestration layer of the *agentic IDE client* itself, not in the reader. The cure for §3.10 is consent-gated memory writes in the agentic client (the same orchestrator-side principle as §5.2.5) and fresh-context session restart to purge already-installed contaminating tokens, not anything the §5.2 reader architecture can provide.
