# Sara as a Measurement Instrument for LLM Behavior: A Reference Substrate for Studying Transformer Failure Modes
...
Large language model research has, until now, operated without a fine-grained measurement instrument for transformer behavior. Researchers can observe output and grade it against natural-language references, but they cannot separate retrieval from synthesis from training recall from context leakage at the token level — because the model's input substrates (training corpus, retrieved documents, conversational history) are either too large to fully inspect, too unstructured to grade against, or both. We show that a persistent, inspectable, triple-structured knowledge substrate — *Sara Brain*, a path-of-thought cognitive architecture (Pearl, 2026a) — functions as a scientific instrument for studying transformer behavior in a way that prior substrates do not.

Sara satisfies four properties simultaneously: it is **finite** (every contained fact is explicitly inspectable), **structured** (each fact is a traceable (subject, relation, object) triple with provenance), **large enough** to support nontrivial questions over multi-hop reasoning, and **orthogonal to training data** (the substrate's contents are not in any public training corpus, because they are loaded at runtime by a human teacher). These four properties, in combination, have not been available in any prior substrate. Retrieval benchmarks (HotpotQA, NaturalQuestions) have natural-language ground truth that cannot be graded per-triple. Knowledge graphs (Wikidata, ConceptNet) are massive and their contents are largely in training data. Synthetic benchmarks (bAbI, CLUTRR) are small and lose realism. Hand-curated private corpora are one-off and not reusable across experiments. Sara's substrate can be built for any domain, inspected exhaustively, and graded against model output at arbitrary granularity.

We demonstrate the instrument in operation by reporting seven distinct findings about transformer behavior, all derived in a single afternoon of experimentation against a 169-triple Sara substrate loaded from the Executive Summary of an unpublished research paper. Each finding is an observation that has been discussed theoretically in the literature but has been difficult to measure cleanly. With Sara as reference, they become measurable.
...
3. **Demonstration.** We report seven distinct findings about transformer behavior — *training corrupts reading, weight is bias in both directions, interpretation-layer bias, session-context contamination, multi-layer failure cascades, smaller ≠ worse for substrate-bound tasks, and token premium dis-correlates from fidelity* — all produced in a single afternoon of experimentation against a 169-triple Sara substrate. Each finding has been theorized or anecdotally observed elsewhere but has been difficult to measure cleanly. With Sara, they become directly measurable.

4. **Method.** We describe the experimental protocol (teach / test-fresh / control-fresh) that the instrument supports, with attention to the contamination traps (notably session-context leakage) that can compromise measurements taken with any such substrate.

### 1.4 Paper organization

Section 2 describes Sara's architecture in the specific aspects that make it suitable as an instrument. Section 3 formalizes the four-property criterion. Section 4 describes the measurement protocol. Section 5 presents the seven demonstrated findings with their supporting evidence. Section 6 discusses generalization — how other researchers could build Sara-style substrates for their own questions. Section 7 addresses limitations. Section 8 reviews related work. Section 9 concludes.
...
Failure to use three separate sessions is a common trap. Pearl (2026e) [2] documents the specific contamination mechanisms: training-bias (weights), interpretation-bias (acute keyword triggers), and session-context accumulation (cumulative). All three can compromise a measurement, but the third is the most insidious because it looks like a measurement that worked.

---

## 5. Demonstrated findings

We report seven findings, all derived on 2026-04-23 from the same 169-triple Sara substrate loaded from the Executive Summary of Pearl (2026c) [3], an unpublished paper on RNA aptamer engineering. Each finding was made visible by the instrument in a way that prior measurement methods would not have supported.
...
This finding would be difficult to establish without a reference substrate. On ingestion you could compare the LLM's extracted facts to the source text, but you'd be comparing prose to prose. On emission without a reference, any "invented" content from the model is indistinguishable from "legitimate synthesis." Sara breaks the symmetry: the substrate contains exactly what the experimenter put there, and anything the model emits that is not in the substrate is, unambiguously, not from the substrate.
...
Full treatment: Pearl (2026e) [2], Case 2.3.

### 5.5 Multi-layer failure cascades

The SNARE failure in §5.3 was not a single-stage error. It cascaded:
...
### 5.6 Smaller ≠ worse for substrate-bound tasks

The common reflex in LLM deployment is "use the best (largest) available model for important tasks." Finding 5.1 inverts this for a specific task class: on substrate-bound tasks (tasks judged by fidelity to an external reference), smaller models are *preferable* readers — not because they are cheaper but because they are more faithful.
...
- **Hallucination research.** Build a substrate on a topic the LLM should not know (a novel dataset, an unpublished paper, a private corpus). Ask questions. Grade the output against the substrate at triple granularity. Deviations are hallucinations, and their patterns (template-matching? paraphrasing? fabrication?) are classifiable.

- **Alignment research.** Build a substrate that contains known-conflicting claims (competing hypotheses, contradictory expert positions). Ask the LLM to navigate them. Observe whether the LLM flattens the conflict, picks sides based on training priors, or represents the conflict faithfully.

- **RAG architecture research.** Build substrates at different structural formats (graph, flat document, hierarchical tree) containing equivalent content. Ask the same questions. Observe which architecture the LLM reads most faithfully.

- **Agent memory research.** Teach an agent new content in Session A, then observe what survives into Session B via its memory system. The substrate serves as ground truth for what *should* have been remembered.

- **Interpretability research.** Probe the LLM's internal representations when given substrate-retrieved content vs. training-recalled content. With a known substrate, activations on retrieved facts can be distinguished from activations on invented facts.
...
The present demonstration uses a 169-triple substrate — enough for the seven findings reported, but small compared to production knowledge graphs. Scaling the instrument to substrates of 10,000 or 100,000 triples requires investment in teach-time tooling and grading automation. This is tractable but not free.
...
bAbI [12], CLUTRR [13], RuleTaker [14] construct small synthetic substrates. These satisfy all four properties in principle but fail Property 3 in practice — the reasoning they admit is artificial and does not stress real-world model behaviors.
...
ACT-R [15], Soul [16], and Sara [1] are cognitive architectures with persistent structured memory. Of these, Sara is purpose-built for cortex-cerebellum LLM integration and has the cleanest triple-grain inspectability interface. Other cognitive architectures may be adapted to serve the instrument role with modification; this is an open direction.
...
[2] Pearl, J. (2026e). *Model Infections: How Training Bias Propagates Within and Across Conversations.* Draft (living document, companion to this paper).
...
[4] Pearl, J. (2026d). *Training Corrupts Reading: Empirical Evidence That Smaller LLMs Retrieve Knowledge Graphs More Faithfully Than Larger Ones.* Draft (companion to this paper).
...
## Appendix A — Full triple listing

The 169 triples used in the §5 demonstration are available in the companion artifact `papers/aptamer_rev1/teach_exec_summary.py` as the `TRIPLES` list. Each triple is preceded by a comment linking it to the source sentence from the paper's Executive Summary (§2).

## Appendix B — Replication recipe

1. Clone the Sara Brain repository; check out the instrument-demonstration commit.
2. Install dependencies and create a fresh Python virtual environment.
3. Run `.venv/bin/python papers/aptamer_rev1/teach_exec_summary.py` to build `aptamer_exec.db`.
4. Confirm `.mcp.json` is present at repo root with `SARA_DB=aptamer_exec.db`.
5. Open a fresh Claude Code session in the repo directory. Approve the sara-brain MCP server. Switch to Haiku 4.5. Ask the evaluation question. Log the response.
6. Open a second fresh Claude Code session. Switch to Opus 4.7. Ask the same question. Log the response.
7. Open a third fresh Claude Code session *in an unrelated directory* (no `.mcp.json`). Ask the same question to Haiku and to Opus. These are the Session C control responses.
8. Grade each logged response against the 169 triples. Deviations, inventions, and paraphrases are the findings.
