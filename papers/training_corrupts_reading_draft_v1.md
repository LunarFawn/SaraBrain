...
**Keywords:** cognitive architecture, knowledge graph retrieval, grounded generation, path-of-thought, LLM reading fidelity, asymmetric inference, confabulation, training bias
...
1. **An empirical demonstration of reading corruption.** On a question answerable entirely from 169 graph triples, a larger LLM (Claude Opus 4.7) invented ~30% of its output content — none of it present in the graph — while a smaller LLM (Claude Haiku 4.5) answered with 100% graph-traceable content at one-tenth the token cost.
...
The source is Pearl 2026c [3], an in-progress unpublished paper titled *"Design rules for short-length RNA aptamer engineering observed in a published Massive Open Laboratory dataset that may give us the ability to render disease inert in the body with a knob turn."* The paper introduces several novel terms the authors coined specifically for this work: *the molecular snare*, *Marker Theory*, *Switch Acceptance Theory*, *the Serena RNA Analysis Tool* and its four metrics (*Ensemble Variation*, *Local Minima Variation*, *Weighted Structures*, *Comparison Structures*), *the 5'3' Static Stem* with its two hypotheses (*Mechanics of Materials* and *Thermodynamics*), *the Fold Signal Region*, *the knob* (the paper's central metaphor), and sublab identifiers *SSNG1*, *SSNG2*, *SSNG3*. None of these terms appears in public training corpora; the paper is not yet on Zenodo or any preprint server.
...
Pearl 2026b established the teaching-side failure: an ingesting LLM cannot parse source text into facts without overlaying training-pattern substitutions. A trained LLM reading Wikipedia into a graph writes things the graph shouldn't contain.
...
The naive single-model pipeline inherits the completion bias on every step. The two-stage pipeline isolates the fidelity-critical step to a substrate with lower completion bias.
...
The empirical basis is two-sided: Pearl 2026b on the teaching side (45 human-taught facts beat 28,373 LLM-ingested facts on MMLU), this paper on the reading side (smaller LLM retrieves graph-bound content more faithfully than larger). A joint paper combining the two findings under the weight-is-bias principle could offer the field a single actionable design rule.
