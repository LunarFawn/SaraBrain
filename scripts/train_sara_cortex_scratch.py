...
class Tokenizer:
    ...
    def encode(self, text, max_len=512):
        ids = [1]
        for tok in self._split(text)[:max_len - 2]:
            ids.append(self.tok2id.get(tok, 3))
        ids.append(2)
        return ids
...
def main():
    ...
    tokenizer.fit(texts, max_vocab=4096)
    print(f"Vocab: {len(tokenizer.tok2id)}")
