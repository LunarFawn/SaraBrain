...
2. **A catalog of seven observed cases** drawn from controlled experiments (§2): five confirmed infections meeting the persistence criterion (§2a) and two related confabulation types (§2b).
...
**Course.** At the time of writing this case, the persistence had not cleared. The author proposed the §5d-prescribed cure — a fresh session — and asked the assistant to choose between continuing in the contaminated session (with active flagging) or restarting. The decision was deferred to allow this case to be documented first. A subsequent fresh session, started with the deleted memory file absent from both disk and context, proceeded without invoking the deleted constraint — consistent with the §5d prescription.
...
Training bias on tool-call selection rather than on content. The model picks a tool whose semantics do not match the question, then misinterprets the result through the wrong tool's lens.
...
**Three of three targeted failure mechanisms were eliminated** in the user-visible output of the stateless architecture (Case 2.6, Case 2.7, and the Case 2.1 keyword-priming class via Q2's molecular-snare retrieval) — n=3, single domain, single substrate.
...
An *empty-string contamination check* is applied to every routing response: any response in which the extracted concept or type field is the empty string is rejected and retried, since this is a common failure mode when the 3B returns a structurally valid JSON object with blank values.
...
This is the long arc and not yet implemented. We offer it as the correct theoretical framing for a class of solution, not as a known-but-unimplemented defense. Whether a grammar-only training corpus can be constructed that preserves language fluency while removing world content is an open empirical question — naturalistic text inevitably contains world content, and a delexicalized syntactic corpus loses the prosody and idiom that make a cortex's prose readable. The path to this solution begins with characterizing how much of the training corpus needs to be removed before the single-shot recall symptoms reduce, and that characterization is itself a research program.
...
Methods that train models to follow detailed protocol instructions (Instruction Tuning [9]) operate on the assumption that more detailed instructions improve compliance. Alignment approaches that harden model behavior through training (Constitutional AI [8], Instruction Tuning [9]) are orthogonal to the structural defense proposed here — they operate on the model's training, whereas the infection framework proposes architectural defenses that operate at inference time. Case 2.7 (format-imitation under hardened protocol instructions) is a counterexample at small model scale: more detailed instructions can *increase* the surface-form compliance while reducing actual-grounding compliance. This suggests that instruction-following capability and substrate-grounding capability are not the same axis.
...
[1] Pearl, J. (2026a). *Path-of-Thought Cognitive Architecture: Cortex-Cerebellum Integration for Language Models.* Zenodo preprint. DOI: 10.5281/zenodo.19436522.
